...
if stock_codes:
    ...
    for stock_code in stock_codes_list:
        ...
        if not df.empty:
            ...
            # 積算が10倍に達した日を強調
            plt.scatter(df.index[df['Cumulative_Pos'] >= 2 * df['SMA5']], df['Close'][df['Cumulative_Pos'] >= 2 * df['SMA5']], color='orange', label='積算 > 2倍 SMA5', marker='o',s=100, zorder=3)
            plt.scatter(df.index[df['Cumulative_Neg'] <= -2 * df['SMA5']], df['Close'][df['Cumulative_Neg'] <= -2 * df['SMA5']], color='purple', label='積算 < -2倍 SMA5', marker='x',s=100, zorder=3)
